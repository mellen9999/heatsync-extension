...
const EFFECTS = {
  ...
  pan:      { slot: 'paint', luminance: false, basePeriod: 5,   letterSplit: false },
  conic:    { slot: 'paint', luminance: false, basePeriod: 6,   letterSplit: false },
  hue:      { slot: 'paint', luminance: true,  basePeriod: 8,   letterSplit: false, sig: 'self:filter' },
  glint:    { slot: 'paint', luminance: false, basePeriod: 3.4, letterSplit: false },
  reveal:   { slot: 'paint', luminance: false, basePeriod: 3,   letterSplit: false },
  stripes:  { slot: 'paint', luminance: false, basePeriod: 2.4, letterSplit: false },
  stardust: { slot: 'paint', luminance: false, basePeriod: 3,   letterSplit: false },
  pulse:    { slot: 'paint', luminance: true,  basePeriod: 2.4, letterSplit: false, sig: 'self:opacity' },

  // ── motion/glow slot — up to 2, distinct sig required ───────────────────
  wave:    { slot: 'motion', luminance: false, basePeriod: 1.6, letterSplit: true,   sig: 'letter:transform' },
  ripple:  { slot: 'motion', luminance: true,  basePeriod: 2.4, letterSplit: true, sig: 'letter:filter' },
  coin:    { slot: 'motion', luminance: false, basePeriod: 5,   letterSplit: false,     sig: 'self:transform' },
  heli:    { slot: 'motion', luminance: false, basePeriod: 2.2, letterSplit: false,          sig: 'self:transform' },
  float:   { slot: 'motion', luminance: false, basePeriod: 5.5, letterSplit: false,  sig: 'self:transform' },
  heart:   { slot: 'motion', luminance: false, basePeriod: 1.3, letterSplit: false,     sig: 'self:transform' },
  wobble:  { slot: 'motion', luminance: false, basePeriod: 2.8, letterSplit: false, sig: 'self:transform' },
  swing:   { slot: 'motion', luminance: false, basePeriod: 2.6, letterSplit: false,      sig: 'self:transform' },
  tumble:  { slot: 'motion', luminance: false, basePeriod: 3.4, letterSplit: true, sig: 'letter:transform' },
  neon:    { slot: 'motion', luminance: true,  basePeriod: 2.6, letterSplit: false,  sig: 'self:shadow' },
  glitch:  { slot: 'motion', luminance: false, basePeriod: 2.8, letterSplit: false,        sig: 'self:shadow' },
  jitter:  { slot: 'motion', luminance: false, basePeriod: 3,   letterSplit: false,        sig: 'self:transform' },
  hop:     { slot: 'motion', luminance: false, basePeriod: 2.2, letterSplit: true,    sig: 'letter:transform' },
  twirl:   { slot: 'motion', luminance: false, basePeriod: 3.6, letterSplit: true,  sig: 'letter:transform' },
  type:    { slot: 'motion', luminance: true,  basePeriod: 4,   letterSplit: true,    sig: 'letter:opacity' },
  flicker: { slot: 'motion', luminance: true,  basePeriod: 3,   letterSplit: false,       sig: 'self:opacity' },
}
...
/**
 * Canonical key for a fill layer's `motion` — two layers with identical motion
 * share one live animation, so they cost ONE slot together rather than one
 * each. Read on BOTH sides of the authoring split: the compiler groups boxes by
 * it (fillBoxOf below) and the save gate charges the effect budget by it
 * (paint-authoring's fillEffectCount). It stays here, on the render path, so
 * the two can never disagree about what shares an animation.
 */
export function motionGroupKey(m) {
...
// Read by utils/paint-authoring.js — the save-time half of this module, split
// out so the validator does not ride the render path into the boot closure.
// These are the enums and builders both halves share; the arrow points one
// way (authoring -> here), and nothing below may import from there.
export {
  MAX_EFFECTS, BASE_TYPES, GLOW_STRENGTHS, MIN_STOPS, MAX_STOPS,
